...
$excludePatterns = @("*.log", "logs", ".git", "*.tmp", "node_modules", ".venv", ".cursorkit", ".precursor", "*.zip", "*.bak")
Get-ChildItem -Path "." -Exclude $excludePatterns | Copy-Item -Destination $tempDir -Recurse -Force

# Copy Precursor into the release (batteries included, single source of truth in repo)
$precursorSource = Join-Path $PSScriptRoot "..\precursor"
$precursorDest = Join-Path $tempDir "precursor"
if (Test-Path $precursorSource) {
    Copy-Item -Path $precursorSource -Destination $precursorDest -Recurse -Force
} else {
    Write-Host "⚠️ Precursor source not found at: $precursorSource" -ForegroundColor Yellow
}
