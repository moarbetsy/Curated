...
function Set-CursorSettings {
  ...
  if (-not (Test-Path $SettingsTemplatePath)) {
    $SettingsTemplatePath = Join-Path $ProjectRoot "scripts\cursor-settings.json"
  }

  # Check if template exists
  if (-not (Test-Path $SettingsTemplatePath)) {
    Log "ERROR: Settings template not found. Looked for cursor-settings.json and scripts\cursor-settings.json in: $ProjectRoot"
    Write-Host "ERROR: Settings template not found. Looked for cursor-settings.json and scripts\cursor-settings.json in: $ProjectRoot" -ForegroundColor Red
    return $false
  }
