...
## Rules single source of truth (rules-src)

If this repository contains `rules-src/`, Precursor enforces that generated Cursor rules are **in sync** with that canonical source.
On `setup` and `scan`, Precursor will fail fast if it detects drift or JSON-wrapped `.mdc` corruption.

Fix drift by regenerating:

- `pwsh ./curated.ps1 gen-rules`
- or `pwsh ./scripts/gen-rules.ps1`
